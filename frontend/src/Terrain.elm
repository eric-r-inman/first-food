...
static dev server is read-only, "save" downloads a regenerated terrain.toml the
...
paletteDecoder =
    D.map2 Tuple.pair
        ...
        (D.field "terrain" (D.list terrainDecoder))
    ...
    case msg of
        ...
        AddTerrain ->
            let
                ...
                terrain =
                    { id = "new_terrain", key = key, glyph = "?", color = "white", props = Dict.empty }
            in
            ( { model | terrains = model.terrains ++ [ terrain ], selected = List.length model.terrains, status = "added terrain" }, Cmd.none )
        ...
        Save ->
            ( { model | status = "saved terrain.toml — drop it in data/ and re-run just editor" }
...
view model =
    div [ A.style "font-family" "monospace", A.style "background" "#0f1115", A.style "color" "#ddd", A.style "min-height" "100vh", A.style "padding" "12px" ]
        [ div [ A.style "font-size" "18px", A.style "margin-bottom" "8px" ] [ text "terrain editor" ]
        , div [ A.style "display" "flex", A.style "gap" "24px", A.style "flex-wrap" "wrap", A.style "align-items" "flex-start" ]
            [ listView model
            , detailView model
            , propsView model
            ]
        , div [ A.style "margin-top" "12px" ] [ plainButton Save "save terrain.toml" ]
...
listView model =
    div []
        [ div [ A.style "opacity" "0.7", A.style "margin-bottom" "4px" ] [ text "terrains" ]
    ...
    case selectedTerrain model of
        ...
        Just t ->
            div [ A.style "min-width" "260px" ]
                ...
                , div [ A.style "margin-top" "8px" ] [ plainButton DeleteTerrain "delete this terrain" ]
                ]
...
propsView model =
    div [ A.style "min-width" "220px" ]
        ...
        , div [ A.style "margin-top" "8px", A.style "opacity" "0.55", A.style "max-width" "220px", A.style "font-size" "12px" ]
            [ text "Adding a property gives it to every terrain as null until you set a value." ]
        ]
