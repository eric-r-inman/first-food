...
paletteDecoder =
    D.map2 Tuple.pair
        ...
        (D.field "resources" (D.list terrainDecoder))
    ...
    case msg of
        ...
        AddTerrain ->
            let
                ...
                terrain =
                    { id = "new_resource", key = key, glyph = "?", color = "white", props = Dict.empty }
            in
            ( { model | terrains = model.terrains ++ [ terrain ], selected = List.length model.terrains, status = "added resource" }, Cmd.none )
        ...
        Save ->
            ( { model | status = "saved resources.toml — drop it in data/ and re-run just editor" }
            , Download.string "resources.toml" "application/toml" (toToml model)
            )
...
view model =
    div [ A.style "font-family" "monospace", A.style "background" "#0f1115", A.style "color" "#ddd", A.style "min-height" "100vh", A.style "padding" "12px" ]
        [ div [ A.style "font-size" "18px", A.style "margin-bottom" "8px" ] [ text "resource editor" ]
        , div [ A.style "display" "flex", A.style "gap" "24px", A.style "flex-wrap" "wrap", A.style "align-items" "flex-start" ]
            [ listView model
            , detailView model
            , propsView model
            ]
        , div [ A.style "margin-top" "12px" ] [ plainButton Save "save resources.toml" ]
    ...
    case selectedTerrain model of
        ...
        Just t ->
            div [ A.style "min-width" "260px" ]
                [ div [ A.style "opacity" "0.7", A.style "margin-bottom" "4px" ] [ text "edit resource" ]
                ...
                , div [ A.style "margin-top" "8px" ] [ plainButton DeleteTerrain "delete this resource" ]
                ]
...
propsView model =
    div [ A.style "min-width" "220px" ]
        ...
        , div [ A.style "margin-top" "8px", A.style "opacity" "0.55", A.style "max-width" "220px", A.style "font-size" "12px" ]
            [ text "Adding a property gives it to every resource as null until you set a value." ]
        ]
